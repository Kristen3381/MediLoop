...
import { useStore, canAcceptReferral, canManageReferral } from '../store/useStore';
import TrackingTimeline from '../components/TrackingTimeline';
import { MessageSquare, Send, ArrowLeft, User, Building, Clock, CheckCircle, Truck, MapPin, CheckSquare } from 'lucide-react';
...
export default function ReferralDetails() {
  ...
  const transitionStatus = async () => {
    ...
    if (!canManageReferral(user?.role)) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
        ...
        <div className={clsx(
          ...
          referral.status === 'Completed' ? "bg-success-light border-success text-success" : "bg-warning-light border-warning text-warning"
        )}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* Right Column: Controls & Messaging */}
                ...
                {isUpdating ? 'Executing...' : (
                  <>
                    <span>Advance to: {
                      ...
                      referral.status === 'Accepted' ? 'In Transit' :
